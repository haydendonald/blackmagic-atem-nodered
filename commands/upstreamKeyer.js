module.exports = {
  ...
  processData(data, flag, command, commandList, sendTallyUpdates=true) {
    ...
    commandList.list.inputProperty.updateTallysKeyer(((data[0] * 10) + (data[1])), "upstreamKeyerTallyFill", this.data[((data[0] * 10) + (data[1]))].fillSource,  data[2] == 0x01, sendTallyUpdates);
    commandList.list.inputProperty.updateTallysKeyer(((data[0] * 10) + (data[1])), "upstreamKeyerTallyKey", this.data[((data[0] * 10) + (data[1]))].keySource,  data[2] == 0x01, sendTallyUpdates);
  ...
  addKeyerInformation(keyerId, fillSource, keySource, commandList) {
    if(this.data[keyerId] != undefined || this.data[keyerId] != null) {
      ...
      commandList.list.inputProperty.updateTallysKeyer(keyerId, "upstreamKeyerTallyKey", this.data[keyerId].keySource, this.data[keyerId].state, false);
    }
  },
