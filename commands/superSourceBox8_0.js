...
module.exports = {
  object: function() { return {
    ...
    processData(data, flag, command, commands, sendTallyUpdates=true) {
      ...
      if(commands.version.getVersion() < commandList.version.V8_0){return false;}
  
      this.data[data[0] + 1][data[1] + 1] = {
        "enabled": data[2] == true,
        "inputNumber": data.readUInt16BE(4),
        "videoSource": commands.inputProperty.findInput(data.readUInt16BE(4)),
          ...
          if(error === null) {
            ...
            if(error === null) {
              ...
              if(commandList.exists(command.payload.data.inputNumber) && commandList.exists(command.payload.data.videoSource)){error="Both inputNumber and videoSource are defined only one is required";}     
                ...
                var input = commands.inputProperty.findInput(search);
