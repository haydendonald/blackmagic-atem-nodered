module.exports = {
  ...
  processData(data, flag, command, commandList) {
    ...
    this.data.soloInput = {
      ...
      "input": commandList.list.audioMixerInput.findMixerInput(this.data.soloInput)
    };
    ...
    if(!commandList.exists(command.payload.data) || command.payload.data === {}) {
      ...
      if(commandList.exists(command.payload.data.soloInput)) {
          if(Number.isInteger(command.payload.data.soloInput)) {
            ...
            var input = commandList.list.audioMixerInput.findMixerInput(command.payload.data.soloInput);
            if(input !== undefined) {
